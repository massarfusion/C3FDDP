...
from minlora import (
    ...
    add_lora,
    apply_to_lora,
    merge_lora, get_lora_params, get_lora_state_dict,
)
...
class Trainer():
    def __init__(self, dataloader, cfg_data, pwd):
        ...
        for param in self.net.CCN.parameters():
            param.requires_grad = False  # it was require_grad
        self.net.CCN = self.net.CCN.cpu()
        add_lora(self.net.CCN, lora_config=lora_config)
        self.net.CCN = self.net.CCN.cuda()
        parameters = [
            {"params": list(get_lora_params(self.net.CCN))},
        ]
        self.optimizer = optim.Adam(parameters, lr=cfg.LR, weight_decay=1e-4)
        print(self.net)
        if cfg.PRE_LORA:
            print(os.getcwd())
            self.net.load_state_dict(torch.load(cfg.PRE_LORA_MODEL), strict=False)
        # Here
    ...
    def forward(self):
        ...
        for epoch in tqdm(range(self.epoch,cfg.MAX_EPOCH)):
            print("You have entered Epoch {}".format(epoch))
            self.epoch = epoch
            ...
            if epoch%cfg.VAL_FREQ==0 or epoch>cfg.VAL_DENSE_START:
                # HERE
                lora_state_dict = get_lora_state_dict(self.net)
                torch.save(lora_state_dict, os.path.join(self.exp_path, self.exp_name, 'LoraWeight.pth'))
                # Here
            ...
            print("You have Finished Epoch {}, congratulations!".format(epoch))
    ...
    def train(self): # training for all datasets
        ...
        for i, data in enumerate(tqdm(self.train_loader)):
            ...
            img, gt_map = data#举例在选用SHHB和Res50时 img(BATCH,3,768,1024) gt_map(BATCH,768,1024)
    ...
    def validate_V1(self):# validate_V1 for SHHA, SHHB, UCF-QNRF, UCF50
        ...
        maes = AverageMeter()
        mses = AverageMeter()
#enumerate(tqdm(self.val_loader))
        for vi, data in enumerate(tqdm(self.val_loader)):
            img, gt_map = data
    ...
    def validate_Individual(self):  # validate_V1 for SHHA, SHHB, UCF-QNRF, UCF50
